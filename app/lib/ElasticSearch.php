...
class ElasticSearch {
  ...
  //curl -X GET http://localhost:9200/{INDEX}/{TYPE}/_search?q= ... [size= ...] [from= ...]
  function query($type, $q, $s = '', $f = ''){
	$query_params = array('q' => $q);
	if ( is_int($s)) $query_params['size'] = $s;
	if ( is_int($f)) $query_params['from'] = $f;
    return $this->call($type . '/_search?' . http_build_query($query_params));
  }
}
